...
var fileTreeVue = new Vue({
    ...
    methods:{
        async sendRequest(userId){
            await axios.get("../../manager/getFileTreeById?userId="+userId).then(resp=>{
    ...
    mounted(){
        ...
        let userId = location.href.substring(index+1);
        this.sendRequest(userId).then(r=>{
...
//     axios.get("http://locahost:8080/FileManageSystem/preview?fileName="+fileId,{"Pragma":"no-cache","Cache-Control":"no-cache"}).then(async resp=>{
